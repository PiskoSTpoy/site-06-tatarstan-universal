<script lang="ts">
	...
	// После волны 35 в блоге 6 статей о том, что определяет дату выезда крана на объект: пять про
	// наряд-допуск на площадку (газоопасная зона и работа у ЛЭП) и одна про допуск на дорогу к ней
	// (спецразрешение) — блок показывает три другие карточки на каждой странице, заголовок
	// учитывает число ("Ещё материалы по теме" во множественном). Врать про "релевантность" не
	// нужно: у всех статей общий предмет — что должно быть оформлено, прежде чем кран доедет и
	// зайдёт на объект, — даже когда конкретное основание разное.
