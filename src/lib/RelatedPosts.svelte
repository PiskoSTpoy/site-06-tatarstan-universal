<script lang="ts">
	...
	// После волны 36 в блоге 7 статей о том, что определяет дату выезда крана на объект: шесть про
	// наряд-допуск или разрешение на площадку (газоопасная зона, работа у ЛЭП, охранная зона
	// газопровода) и одна про допуск на дорогу к ней (спецразрешение) — блок показывает три другие
	// карточки на каждой странице, заголовок учитывает число ("Ещё материалы по теме" во
	// множественном). Врать про "релевантность" не нужно: у всех статей общий предмет — что должно
	// быть оформлено, прежде чем кран доедет и зайдёт на объект, — даже когда конкретное основание
	// разное.
