<script lang="ts">
	...
	const title = 'Блог: наряд-допуск на кран в Татарстане — разборы';
	const description =
		'Пять статей одной линии: когда крану нужен наряд-допуск — из-за газоопасной зоны или ЛЭП, по какому регламенту его оформляют, что грозит за нарушение и кто согласовывает.';
...
<main>
	...
	<section class="section wrap" style="padding-top:8px">
		<div class="section-head">
			...
			<h1>Пять статей вместо двадцати</h1>
			<p>
				Блог здесь узкий намеренно: все 5 статей разбирают один документ — наряд-допуск, и то, что делает его
				обязательным на объекте: газоопасную зону предприятия или соседство с линией электропередачи. Это
				процедуры, которые на площадках нижнекамской нефтехимии и на любом объекте с ЛЭП рядом определяют дату
				выезда техники.</p>
			<p>Первые две отвечают
				на «что это» и «по какому регламенту», следующие две — на «что будет, если нарушить» и «кто, кроме
				предприятия и подрядчика, в этом участвует», а пятая — на «а если триггер вообще не химия, а линия
				электропередачи».</p>
	...
	<section class="section wrap prose" style="padding-top:0;max-width:840px">
		...
		<ul>
			<li>
				...
				согласовывает наряд со стороны газоспасательной службы и когда нужен отдельный наряд из-за близости
		...
		<p>
			...
			Пятая статья блога опирается на другой пункт того же документа (п. 112, наряд у ЛЭП) не потому, что мы
			изменили этому принципу, а потому, что там, в отличие от освидетельствования, есть живая региональная
			сторона согласования — конкретная сетевая организация, а не общий текст правил. Нет и разбора ветровых
			ограничений: предельная скорость для конкретной машины стоит в её паспорте и приоритетна над любым общим
			числом, так что статья тут заменяется одной строкой. Пересказ норматива без регионального слоя — это не
			статья, а дубль первоисточника, только с риском опечатки в цифре.
		</p>
		<p>
			Что осталось нашим: наряд-допуск и всё, что делает его обязательным на конкретном объекте, — газоопасная
			зона предприятия или соседство с линией электропередачи, — потому что за этим стоит реальная региональная
			практика, а не поисковый запрос. Республика с концентрацией нефтехимии в Нижнекамске и собственной сетевой
			организацией даёт достаточно материала, чтобы писать об этом от первого лица.
		</p>
