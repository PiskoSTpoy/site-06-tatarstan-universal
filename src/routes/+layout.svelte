<script lang="ts">
	...
	// 08.09.2026 — отложенная загрузка до window.load (фоллбэк 4с), тот же паттерн, что и на
	// kranneva.ru/kran365.ru (сеть, см. память kran-network-core-web-vitals): webvisor:true —
	// тяжёлая опция, tag.js ощутимо грузит главный поток на throttled CPU, а к моменту load
	// LCP/FCP уже случились. Флаг __ymBooted — от двойного срабатывания (load + таймаут).
	// ym() на этом сайте больше нигде не вызывается (grep по src/ — ноль совпадений).
	const METRIKA_ID = 112124215;
	const metrikaSnippet =
		'<scr' + 'ipt type="text/javascript">' +
		"function __ymBoot(){if(window.__ymBooted)return;window.__ymBooted=true;" +
